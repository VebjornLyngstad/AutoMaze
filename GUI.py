...
#time.sleep(7.5) #Give it time to connect to serial communication when starting the program on boot
...
def button1_function():
    ...
    display_text = """
            ...
            • When returning to main menu, also press any Xbox button
            """
...
def update_dynamic_text():
    ...
    if detected_maze:
        dynamic_text.configure(text=f"Maze detected: {detected_maze}", text_color="green", font=("Comic Sans MS", 28, "bold"),)
        run_auto_button.configure(state=tk.NORMAL)  # Enable the "Run Auto Control" button
    else:
        dynamic_text.configure(text="No maze detected, can't run auto control", text_color="red", font=("Comic Sans MS", 28, "bold"))
...
def run_auto_control():
    ...
    if detected_maze:
        ...
        print("Cannot run auto control, no maze detected.")
